...
//for delete
  const handleyesno = async (id) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/api/comingsoon/delete/${id}`, {
        method: "DELETE",
      });
...
//for status
  const handleChangestatus = async (status,id) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/api/comingsoon/update-status`, {
        method: "PATCH",
        headers: {
      ...
      if (response.ok == true) {
        fetchData();
        toast.success('Coming Soon Status updated Successfully');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/api/comingsoon/listdata`, {
        method: "GET",
      });
    ...
    },
    {
      ...
      cell: row => (
        <div className="module-action">
          ...
          <Link to={`/coming-soon/edit/${row._id}`} className="btn btn-action" title="Edit Package">
            <FaEdit />
          </Link>
          ...
          <Link to={`https://comingsoon.digihostinfra.com/${row._id}`} className="btn btn-action" target="__blank" title="Coming Soon Page">
            <FaEye />
        </Link>
        </div>
      ),
          ...
          <Card className="create_new_page_card add-module module-list">
            <Card.Header>
              <Row className="justify-content-between">
                <Col md={12}>
                  <h3>Coming Soon Page List test </h3>
                </Col>
                <Col md={6}>
                </Col>
